...
def decode_files(color_enc, depth_enc, i):
    ## RGB file decode  
    ...
    if frame_depth_decoded is None:
        logger.error("Decoded color image is None. Error loading image or corverting color image to RGB. Check the data format")
    cv2.imwrite(f"depth_{i}.png", frame_depth_decoded)
...
def process_frames(msg_frames_list, num_frame, container_name, K):
    ...
    for message in msg_frames_list: # processing el batch
        ...
        frame_color_decoded, frame_depth_decoded = decode_files(color_file, depth_file, i) # tipo de varible: NUMPY.NDARRAY 
        frame_depth_decoded[frame_depth_decoded==0]=255
        cv2.imwrite(f"premaskdepth_{i}.png", frame_depth_decoded)
        # Append images
    ...
    for imagen, resultado, profun in zip(color_array, results, depth_array):
        ...
        segmentation = show_result_pyplot(model, imagen, resultado, opacity = 1, draw_gt = False, show = False, with_labels=False, save_dir="datafiles", out_file=f"datafiles/segmented_{i}.png")  
        ...
        cv2.imwrite(f"color_img_{i}.png", color_img)
        profun[~mask] = 0 # Depth frame with black background 
        cv2.imwrite(f"profund_a_{i}.png", profun)
    ...
    print(f"Execution time: {end - start_time:.6f} seconds")
