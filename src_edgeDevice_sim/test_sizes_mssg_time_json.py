...
def test_png(file_path_c, file_path_d):
    ...
    time_json = time.perf_counter() - time_start
    print(f"Time to encode files {time_json}")

    return base64_c, base64_d 

# Function to construct and send message to broker hosted in HiveMQ
def build_publish_encoded_msg(client, camera_name, k, color_name, depth_name, dataset_id, container_name, total_cameras, base64_c, base64_d):
...
def process_frames_of_a_camera(client, k_dict, camera_name_path, dataset_id, container_name, total_cameras, downsample_factor): 
    ...
    for chosen_color_frame, chosen_depth_frame in zip(sorted(color_frames), sorted(depth_frames)):
        base64_c, base64_d  = test_png(os.path.join(path_color, chosen_color_frame), os.path.join(path_depth, chosen_depth_frame))
        build_publish_encoded_msg(client, camera_name, k_list, chosen_color_frame, chosen_depth_frame, dataset_id, container_name, total_cameras, base64_c, base64_d)
